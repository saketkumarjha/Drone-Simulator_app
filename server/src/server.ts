...
app.get("/", (req: Request, res: Response) => {
  res.send("Drone Simulator API is running!");
});
...
app.post("/api/upload-coordinates", async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) {
      return res
        .status(500)
        .json({ error: `Error saving file: ${error.message}` });
    }
...
function parseCoordinateFile(content: string, filename: string): Coordinate[] {
  ...
  if (extension === ".json") {
    ...
    throw new Error(
      "Unsupported file format. Please upload JSON, CSV, or TXT files."
    );
  }
}
    ...
    if (Array.isArray(data)) {
      return data.map((point) => {
        if (typeof point.lat !== "number" || typeof point.lng !== "number") {
          throw new Error(
            "Invalid coordinate format. Expected {lat, lng} objects."
          );
        }
    ...
    // Handle GeoJSON format
      ...
      return data.features.map((feature: any) => {
        if (!feature.geometry || !Array.isArray(feature.geometry.coordinates)) {
          throw new Error(
            "Invalid GeoJSON format. Expected coordinates array in geometry."
          );
        }
      ...
      throw new Error(
        "Invalid JSON format. Expected array of coordinates or GeoJSON."
      );
    }
  } catch (error) {
    const errorMessage =
